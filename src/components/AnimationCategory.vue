<template>
  <el-main >
    ...
    <el-row :gutter="5" style="margin-top: 10px;margin-bottom: 10px" >
      <el-col :span="4" v-for="film in animationDataList" :key="film" style="margin-top: 5px">
        <router-link :to="{name:'FilmInfo',params:{url:film.url, img:film.img, title: film.title}}" :style="{textDecoration:'none'}" >
          <el-card shadow="hover" :body-style="{ padding: '0px' }">
            <div style="position:relative;">
              <img class="item_img" v-bind:src="film.img" alt="暂无封面">
              <div>
                <el-tag  class="item_star" >{{film.star}}分</el-tag>
                <span class="item_update">{{film.update}}</span>
              </div>
            </div>
            <div class="item_detail">
              <p  class="item_title">{{film.title}}</p>
              <p class="item_info">{{film.info}}</p>
            </div>
          </el-card>
        </router-link>
      </el-col>
...
export default {
  ...
  data () {
    return {
      ...
      currentTypeName: '全部',
      year: [], // 年代
      animationDataList: [] // 电影数据
    }
  },
  mounted: function () {
    this.$axios.post(global_.serverUrl + 'animation/findAnimationList', {'pageNo': 1}).then(result => {
      this.animationDataList = result.data.animationDataList
    })
    this.$axios.post(global_.serverUrl + 'animation/findCategoryList').then(result => {
  ...
  methods: {
    ...
    handleCurrentChange (val) {
      // console.log("当前页:"+ val);
      this.$axios.post(global_.serverUrl + 'animation/findAnimationList', {'pageNo': val}).then(result => {
        this.animationDataList = result.data.animationDataList
    ...
    switchType (index, types) {
      ...
      this.$axios.post(global_.serverUrl + 'film/findFilmList', params).then(result => {
